...
首页也可切换到“查找作者”，使用作者主页链接加入抖音或知乎作者库。
小红书仅支持单作品下载，不提供作者批量下载。任务页的“作者”页签可查看资料、公开作品和
下载记录，支持跨页选择、当前页全选以及统一设置后批量下载；作者下载记录也支持批量重下和
...
给出明确结果，不会无限停留在准备界面。也可以在首页点击各平台的登录状态，打开对应的登录环境。

清晰度列表优先显示接口或 CDN 返回的精确文件大小；无法取得精确值时显示根据码率和
时长计算的“约”值，仍无法计算时明确显示“大小未知”，不会再显示 0 MB。

抖音和小红书作品包含实况照片时，应用会保留原始静态图片和动态 MP4；静态源为 JPEG 时还会
额外生成 Motion Photo。合成不重新编码，失败只记为警告，不会删除已经下载的两个源文件。
...
设置页会在应用启动后每天最多检查一次 GitHub Release，只更新“发现新版本”提示，不会自动
下载。用户点击更新后可以选择 `gh-proxy.org` 镜像或 GitHub 原始源；选择 GitHub 时应用会提示
未开启代理可能速度慢或失败。下载完成后必须通过包名、版本号、APK 签名证书以及 Release 可用的
SHA-256 校验，才会打开系统安装器。“安装未知应用”权限仅在安装更新时需要。

任务卡片会显示平台返回的作者信息。抖音号直接取公开作者字段；小红书号会在笔记未返回
时以短超时访问作者主页进行尽力补全，失败不会影响作品下载；知乎提供作者昵称复制。任务
支持单个或批量重新下载。应用会先将新内容完整写入独立任务目录，成功后才删除该任务登记的
旧下载文件；下载失败或取消时会保留原文件。旧任务文件夹中的其他
文件仍会保留。
...
- 小红书作者主页和批量下载已停止支持；已有小红书作者缓存不会删除，单作品任务与下载文件
  不受影响。抖音和知乎继续支持作者页及批量下载。
